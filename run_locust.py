...
def main():
    ...
    for experiment in experiment_list:
        for routing_rule in routing_rule_list:
            ...
            #     executor.submit(utils.run_command, )
            # print("All clients have completed.")
            ###########################################################
            
            # utils.run_command(f"locust -f locust/test2.py --host=http://node5.slate-gm-c6420.istio-pg0.clemson.cloudlab.us:32457 --processes 16")
            inp = input("\n@@ Input to extract files from the slate-controller: \n")
            # utils.run_command("ps aux | grep locust")
            ...
            if mode == "profile":
                ...
                if "WATERFALL" in routing_rule or "SLATE" in routing_rule:
                    ...
                    if routing_rule == "SLATE-with-jumping-global":
                        flist = ["hillclimbing_distribution_history.csv", "global_hillclimbing_distribution_history.csv", "jumping_routing_history.csv", "jumping_latency.csv", "region_jumping_latency.csv", "continuously_profiled_traces.csv", "continuous_coef_dict.csv", "body_traces.csv", "body.csv", "body2.csv", "spans.csv", "body3.csv"]
                        for file in flist:
                            src_in_pod = f"/app/{file}"
