...
def main():
    ...
    cachethrash_path = {
        "getRecord": "/getRecord/" ,# todo aditya??
    ...
    ####################################################################
    experiment_list = []
    benchmark_name="cachethrash" # a,b, 1MB and c 2MB file write
    method = "GET"
    req_type = "getRecord"
    total_num_services = 2
    
    # west_rps = [100]
    west_rps = [100,200,300,400,500,600,700,800]
    central_rps = [0]
    south_rps = [0]
    east_rps = [0]
    hillclimb_interval = 30
    duration = [180] * len(west_rps)
    ...
    for w_rps, dur in zip(west_rps, duration):
        ...
        experiment.add_workload(utils.Workload(cluster="west", req_type=req_type,  rps=[w_rps], duration=[dur], method=method, path=cachethrash_path[req_type], endpoint=experiment_endpoint))
