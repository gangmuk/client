...
def plot_latency_cdf(merged_df_list, input_dir):
    ...
    for subdir_name, merged_df in merged_df_list.items():
        # Filter latencies for SLATE-with-jumping-global
        if subdir_name == "SLATE-with-jumping-global":
            filtered_latencies = merged_df[merged_df["Request Latency (ms)"] <= 100]["Request Latency (ms)"].sort_values()
        else:
            filtered_latencies = merged_df["Request Latency (ms)"].sort_values()
            
        cdf = filtered_latencies.rank(method='max').div(len(filtered_latencies))
        avg_latencies[subdir_name] = filtered_latencies.mean()
        p50_latencies[subdir_name] = filtered_latencies.quantile(0.5)
        p90_latencies[subdir_name] = filtered_latencies.quantile(0.9)
        p99_latencies[subdir_name] = filtered_latencies.quantile(0.99)
        p999_latencies[subdir_name] = filtered_latencies.quantile(0.999)
        plt.plot(filtered_latencies, cdf, label=f"{subdir_name}", linewidth=2)
